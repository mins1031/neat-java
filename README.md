...
Hashcode, equals
해시코드와 이퀄스는 페어로 구현되어야 하고 따로 구현되선 안된다.
속도 때문인데 학생을 예로 들면 학생은 10글자인 학번과 이름이 있다. 학번으로 학생을 구분하려면 10글자나 비교해야해 속도가 느리다. 이름으로 학생을 구분하면 3글자라 비교적 쉽다.
그러나 이름은 동명이인이 있을수도 있기에 함정이 있다. 그렇다면 속도를 최적화 하기 위해선?
1. 먼저 이름으로 두학생을 비교해 이름이 다르면 다른학생이다
2. 이름이 같다면 그때 학번을 비교해 판단한다 이름은 같은데 학번이 다르면 다른학생, 둘다 같다면 같은 학생이다
여기서 이름은 해시코드, 학번은 이퀄스다. 
즉 해시코드를 먼저보고 이퀄스를 보는 것은 해시코드로 비교하는게 이퀄스로 비교하는 것 보다 훨씬 빠르기에 해시코드로 걸러낼수 있는걸 구지 이퀄스로 걸러내는 것보다 낫다.
